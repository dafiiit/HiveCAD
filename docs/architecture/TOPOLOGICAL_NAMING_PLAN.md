...
### Phase 2: Sketch Entity Tagging (Week 2-3)

**Goal**: Assign stable IDs to all sketch primitives.

#### Changes:

1. **`src/store/types.ts`** - Add `stableId` to SketchPrimitive:
   ```typescript
   export interface SketchPrimitive {
     id: string;
     stableId: string; // NEW: UUID that persists across edits
     type: string;
     // ...
   }
   ```

2. **`src/lib/sketch-graph/Graph.ts`** - Tag edges with stable IDs:
   ```typescript
   export class GraphEdge {
     stableId: string; // NEW: Persistent ID
     // ...
   }
   ```

3. **Sketch Tools** - Generate stable IDs on primitive creation:
   - `src/lib/tools/core/sketch/line/index.ts`
   - `src/lib/tools/core/sketch/arc/index.ts`
   - etc.

---

### Phase 3: Operation Instrumentation (Week 3-4)

**Goal**: Record topology creation during operations.

#### Key Insight:

When an **Extrude** operation runs:
- Input: Sketch with edges `[edge-A, edge-B, edge-C]`
- Output: Solid with faces `[face-1, face-2, face-3, face-4, face-5]`

We need to record:
- `face-1` (top cap) ← generated by entire sketch profile (semantic: "top")
- `face-2` (bottom cap) ← generated by entire sketch profile (semantic: "bottom")
- `face-3` (side) ← extruded from `edge-A`
- `face-4` (side) ← extruded from `edge-B`
- `face-5` (side) ← extruded from `edge-C`

#### Files to Create:

1. **`src/lib/topology/operations/ExtrudeAnalyzer.ts`**
   - Analyzes an extrusion result and creates StableIds
   - Maps sketch edges to side faces
   - Identifies top/bottom caps

2. **`src/lib/topology/operations/RevolveAnalyzer.ts`**
   - Similar for revolve operations

3. **`src/lib/topology/operations/BooleanAnalyzer.ts`**
   - Tracks face origins through fuse/cut/intersect
   - Handles face splitting and merging

4. **`src/lib/topology/operations/FilletChamferAnalyzer.ts`**
   - Tags new fillet faces with reference to source edge

#### Files to Modify:

1. **`src/workers/replicad-worker.ts`**
   - Hook into `generateMesh` to assign StableIds
   - Return StableId mappings along with mesh data

2. **`src/store/slices/objectSlice.ts`**
   - Store StableId mappings in CADObject
   - Update selection logic to use StableIds

---

### Phase 4: Reference Resolution (Week 4-5)
